...
V3::VanillaTruceEntry::VanillaTruceEntry(std::istream& theStream, std::string sourceTag): sourceTag(std::move(sourceTag))
{
	registerKeys();
...
void V3::VanillaTruceEntry::registerKeys()
{
	...
	registerKeyword("months", [this](std::istream& theStream) {
		duration = commonItems::getInt(theStream);
	});
