...
void OUT::exportCustomCoAs(const std::filesystem::path& outputName, const std::map<std::string, std::shared_ptr<V3::Country>>& countries)
{
	std::ofstream output("output" / outputName / "common/coat_of_arms/coat_of_arms/99_custom_coas.txt");
	if (!output.is_open())
		throw std::runtime_error("Could not create " + outputName.string() + "/common/coat_of_arms/coat_of_arms/99_custom_coas.txt");
