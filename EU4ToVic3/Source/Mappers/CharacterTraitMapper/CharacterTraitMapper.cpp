...
std::set<std::string> mappers::CharacterTraitMapper::getSkillTraits(const EU4::Character& eu4Character) const
{
	...
	if (!eu4Character.leaderType.empty())
	{
		...
		else if (const auto& traitMatch = matchLeaderSkill(eu4Character.leaderType, std::vector<int>{eu4Character.fire, eu4Character.shock, eu4Character.maneuver, eu4Character.siege});
					traitMatch)
